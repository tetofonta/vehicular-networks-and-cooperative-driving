...
namespace plexe::vncd {
    ...
    void TrafficManager::insertCars() {
        struct Vehicle cur_vehicle{
                ...
                .position = 5.0f,
                .speed = 36 //(float) par("platoonInsertSpeed").doubleValue(),
        };
        ...
        if (--this->nCars) {
            scheduleAfter(par("insertDelay").doubleValue() + 6, /*+ (15/cur_vehicle.speed),*/ this->evt_startInsertion.get());
            this->prev_lane = cur_vehicle.lane;
        }
    }
